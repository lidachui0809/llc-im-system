...
//        if(responseVO.isOk()){
        threadPoolExecutor.execute(() -> {
            ...
            if (clientInfos.isEmpty()) {
                ...
                //目标客户不存在时 保存离线数据
                storeOfferlinMsg(messageContent);
            }
...
//        }
    ...
    private void storeOfferlinMsg(MessageContent messageContent) {
    ...
    public void reciverAck(MessageContent messageContent) {
        ...
        logger.info("对方不在线 --- 服务端回复ack msgId={}",messageContent.getMessageId());
    }
