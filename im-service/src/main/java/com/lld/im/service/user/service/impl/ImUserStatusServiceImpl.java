...
public class ImUserStatusServiceImpl implements ImUserStatusService {
    ...
    /**
     * 设计思路 tcp层再接收到客户端登录请求后 会给逻辑层发送一个登录通知 逻辑层接收到消息
     *          将登录消息同步给自己的不同端 以及自己好友和订阅自己的客户端
     * @param content
     */
    @Override
    ...
    /**
     * @description:  设计思路 这里 会将订阅该用户状态的用户id暂存在redis中
     *                          当被订阅用户状态发生改变时 会通过通过appId+beSubUserId为key的方式
     *                          获得订阅用户 最后以此分发给用户
     * @param
     * @return void
     */
